...
class Client {
   ...
   * @param {string} [options.api_key] - Deprecated: use apiKey
   * @param {string} options.apiKey
   * @param {string} [options.apihost]
   * @param {string} [options.namespace]
   * @param {string} [options.ignore_certs] - Deprecated: use ignoreCerts
   * @param {string} [options.ignoreCerts]
   * @param {string} [options.apigw_token] - Deprecated: use apigwToken
   * @param {string} [options.apigwToken]
   * @param {string} [options.apigw_space_guid] - Deprecated: use apigwSpaceGuid
   * @param {string} [options.apigwSpaceGuid]
   */
  constructor (options) {
  ...
  parseOptions (options) {
    if (options.api_key) {
      console.warn('Client api_key is deprecated use apiKey instead.')
    }
    const apiKey = options.api_key || options.apiKey || process.env['__OW_API_KEY']
    if (options.ignore_certs) {
      console.warn('Client ignore_certs is deprecated use ignoreCerts instead.')
    }
    const ignoreCerts = options.ignore_certs || options.ignoreCerts || false
    ...
    if (options.apigw_token) {
      console.warn('Client apigw_token is deprecated use apigwToken instead.')
    }
    const apigwToken = options.apigw_token || options.apigwToken || process.env['__OW_APIGW_TOKEN']
    if (options.apigw_space_guid) {
      console.warn('Client apigw_space_guid is deprecated use apigwSpaceGuid instead.')
    }
    let apigwSpaceGuid = options.apigw_space_guid || options.apigwSpaceGuid || process.env['__OW_APIGW_SPACE_GUID']
    ...
    if (!apiKey) {
      throw new Error(`${messages.INVALID_OPTIONS_ERROR} Missing apiKey parameter.`)
    } else if (!api) {
